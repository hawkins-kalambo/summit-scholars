...
type TutorApplication = {
  ...
  status: string; reviewed_by: string | null; revision: number; created_at: string; tutor_vacancies: { title: string } | null;
};
...
export default async function TutorApplicationsPage() {
  ...
  const [reviewResult, approvedResult] = await Promise.all([
    canReview ? db.from("tutor_applications").select("*,tutor_vacancies(title)").in("status", ["submitted", "under_review"]).order("created_at") : { data: [] as TutorApplication[], error: null },
  ...
  const documentsResults = await Promise.all(applications.map(application => db.from("tutor_application_documents").select("id,file_name,document_category").eq("application_id", application.id)));
  return <div className="dash">
    ...
    {canReview && <>
      ...
      {applications.map((application, index) => {
        const documents = (documentsResults[index].data ?? []) as { id: string; file_name: string; document_category: string }[];
        return <section className="account-panel" key={application.id}>
          ...
          <dl className="detail-list">
            <dt>Vacancy</dt><dd>{application.tutor_vacancies?.title ?? "—"}</dd>
            ...
            <dt>Documents</dt><dd>{documents.length ? <ul>{documents.map(doc => <li key={doc.id}>{doc.document_category.replaceAll("_", " ")}: {doc.file_name}</li>)}</ul> : "None uploaded"}</dd>
          </dl>
