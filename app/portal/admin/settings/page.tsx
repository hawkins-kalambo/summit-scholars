...
export default async function SettingsPage() {
  ...
  return <main className="account-content"><Link href="/portal/home/admin">Admin overview</Link><h1>Organisation settings</h1><ManagedForm action={saveOrganisation} label="Save organisation settings"><label>Organisation name<input name="name" required minLength={2} maxLength={200} defaultValue={data.organisation_name}/></label><label>Support email<input name="email" type="email" required maxLength={254} defaultValue={data.support_email}/></label><label>Reason<textarea name="reason" required minLength={5} maxLength={2000}/></label></ManagedForm>
    <h2>Integration configuration</h2><p>These checks confirm settings are present; they do not test delivery or scheduler operation.</p><ul>{services.map(([label,configured])=><li key={label}>{label}: {configured ? "Configured" : "Missing configuration"}</li>)}</ul><p><Link href="/portal/academics">Manage admissions, privacy notice and academic settings</Link></p></main>;
}
