...
export default async function ApplicationsPage({ searchParams }: { searchParams: Promise<{ page?: string }> }) {
  ...
  return <div className="dash">
    <div className="dash-title"><div><span className="eyebrow">Admissions</span><h1>{staff ? "Applications" : "My applications"}</h1><p>{staff ? "Open an application to review its details, documents and history." : "Save a draft, add supporting documents and track your admission decision."}</p></div>
      {account.roles.includes("student") && <Link className="btn teal" href="/portal/applications/new">Start an application</Link>}
    </div>
