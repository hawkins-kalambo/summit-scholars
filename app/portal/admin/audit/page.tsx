...
export default async function AuditPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return <div className="dash">
    <div className="dash-title"><div><span className="eyebrow">Administration</span><h1>Audit history</h1><p>Read-only records of account, academic, admissions and library changes.</p></div></div>
    <form className="admin-search"><label>Table name<input name="table" defaultValue={table} placeholder="library_resources" pattern="[a-z_]*"/></label><button className="btn teal">Filter</button></form>
