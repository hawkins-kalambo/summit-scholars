...
export default async function PortalLogin({ params }: { params: Promise<{ portal: string }> }) {
  ...
  const description = portal === "student" ? "Sign in to apply and access your student account."
    : portal === "finance" ? "Sign in with your assigned finance account."
    : "Sign in with your assigned " + (portal === "admin" ? "administrator" : "staff or tutor") + " account.";
  return <AuthShell portal={portal} title={portalLabels[portal] + " sign in"} description={description}>
